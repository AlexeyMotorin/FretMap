...
struct FretboardView: View {
    ...
    private func markerViews(boardHeight: CGFloat, layout: FretLayout) -> some View {
        ForEach(markers.filter { layout.contains(fret: $0.position.fret) }) { marker in
            NoteMarker(label: marker.label, isRoot: marker.isRoot, isOpenString: marker.position.fret == 0, markerColor: marker.color)
...
struct NoteMarker: View {
    ...
    private var backgroundColor: Color {
        markerColor ?? AppColors.noteMarker
    }

    private var textColor: Color {
        markerColor == nil ? (isRoot ? AppColors.rootText : AppColors.noteText) : .white
    }

    private var strokeColor: Color {
        if isOpenString {
            return AppColors.openStringStroke
        }
        if markerColor != nil, isRoot {
            return AppColors.rootText
        }
        return .clear
    }
